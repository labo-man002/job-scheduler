...
- **`node_resource` tracks status per resource unit (`resource_status_id`, `resource_type_index`), not `used_capacity`/`total_capacity` counters.** Each physical unit (CPU core, GPU, etc.) gets its own row with its own status, so "which exact unit got allocated" is answerable directly instead of being inferred from a counter.
...
- **The organizational entity is `institute`, not `group`.** `client` already had an `institute` field in the class diagram — the ERD's `group` table was the same concept under a different name, so it's renamed to line up: `client.institute_id`, `quota.institute_id`, `reservation.institute_id`, and `resource_usage.institute_id` all reference `institute`. Quotas, reservations, and usage tracking are institute-level concerns, not per-client ones.
